...
#                           1.2 Load functions                         
#------------------------------------------------------------------------------- 
here::i_am("scripts/simulations/main_sec5_example3.R")
...
#--------------------- main function 3 ------------------------
test3 <- function(X_init,Y_init,X_tr,Y_tr,X_te,Y_te,b=b,h=h,s_vec=s_vec,epsilon=epsilon,delta=delta){
  ##### Inputs ##### 
  ...
  cost_ret <- (b+h)*MQL(Y_te -Y_te_pred,tau)   # Newsvendor-style cost (retailer's cost)
  
  hat_q <- quantile(Y_te_pred, probs = tau, type = 1)                                 # Empirical tau-quantile of predicted order
  cost_sup <- mean(b * pmax(Y_te_pred - hat_q, 0) + h * pmax(hat_q - Y_te_pred, 0))   # SAA cost based on hat_q (supplier's cost)
  
  return( c(cost_sup,cost_ret) )                                 # Return  supplier's cost, retailer's cost
}
...
epsilon_vec <- seq(0.1,0.9,by=0.05)           
n <- 1000 
p <- 50
b <- 2                                                          
h <- 1  
...
for(epsilon in epsilon_vec){
  ...
  re_vec <- mclapply(1:repetitions, function(r){            # require 'parallel'
    ...
    Y <- (itcp + X %*% beta_star) +  Noise                                                      # Generate response Y 
    ...
    re <- test3(X_init, Y_init, X_tr,  Y_tr, X_te,Y_te,   b=b,h=h, s_vec=s_vec, epsilon=epsilon,delta=delta)    # Run evaluation
    return(re)                                                                                  # Return supplier's cost, retailer's cost
    
  }, mc.cores = Ncores)  # number of parallel cores  
  
  Re_1_num <- colMeans(t(matrix(unlist(re_vec), nrow = 2)))  # Return four costs: ell_0 ERM, cs-ERM, Oracle, ERM
...
output_file <- file.path( output_dir, "Example3.csv" )
